...
Built today: `cmd/server/` (wiring) + `internal/{config,store,httpx,logx,types,auth,users,friends,profiles,messaging,sync,nodes}`. Planned per `SERVER.md`: `internal/{sharing,events,rooms,realtime}`.
