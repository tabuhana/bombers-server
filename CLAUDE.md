...
Built today: `cmd/bombers/` (wiring) + `internal/{config,store,httpx,logx,types,auth,users,friends,profiles,messaging,sync,nodes,nodeshare,media,console,setup,embeddedpg,migrate,svc}` (`svc` wraps `kardianos/service` for the P5 background-service mode). Planned per `SERVER.md`: `internal/{sharing,events,rooms,realtime}`.
