...
Built today: `cmd/server/` (wiring) + `internal/{config,store,httpx,types,auth,users,friends,profiles,messaging,sync}`. Planned per `SERVER.md`: `internal/{sharing,events,rooms,realtime}`.
