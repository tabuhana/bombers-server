...
- `nodes/` — the **node store** (`handler.go`/`store.go`, table `nodes`). Nodes are the client's installable, user-buildable deck units; the server hosts their catalog + bundles. `GET /nodes` (catalog metadata, no bytes), `GET /nodes/{id}/bundle` (the `node.js` ESM bytes, integrity hash in the `ETag`), `POST /nodes` (publish/replace — metadata + base64 `bundle`; the server computes the bundle's sha256, rejects a mismatched supplied `hash`, stores the computed one; 4 MiB cap). Bundles are stored as Postgres **`bytea`** — a deliberate carve-out from the no-blob-in-v1 rule (small *code*, not heavy user media). Publishing is auth-gated but **not yet owner-gated** (any authed user can publish), and permissions are **stored as the client sends them** (the client auto-detects them; server re-derivation is a follow-up).
- `console/` — the **interactive admin console** the binary runs on stdin by default (Minecraft-style `bombers>` prompt; skip with `--headless`): a small command registry + loop — `help`, `users` (username/id/created), `status` (uptime, DB ping, row counts), `stop` (aliases `quit`/`exit`) → graceful `http.Server.Shutdown`. Local-operator-privileged by definition (whoever holds the terminal — no auth); the built-ins stay **read-only + stop**; destructive/admin-role commands (delete user, an `is_admin` column, env admin bootstrap) are a deliberate LATER follow-up. A non-TTY stdin or console EOF falls back to headless signal-waiting (never spins). `main.go` serves via `http.Server` in a goroutine and shuts down gracefully in every mode.
