...
Built today: `cmd/bombers/` (wiring) + `internal/{config,store,httpx,logx,types,auth,admin,apitokens,users,discord,settings,friends,profiles,messaging,sync,nodes,nodeshare,packs,activities,releases,rooms,media,console,setup,embeddedpg,migrate,svc}` (`svc` wraps `kardianos/service` for the P5 background-service mode). Planned per `SERVER.md`: `internal/{sharing,events,rooms,realtime}`.
